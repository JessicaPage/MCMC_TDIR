...
L_3_here = L_3_new-L_3_real
L_2_here = L_2_new-L_2_real
L_1_here = L_1_new-L_1_real
L_3_p_here = L_3_p_new-L_3_p_real
L_2_p_here = L_2_p_new-L_2_p_real
L_1_p_here = L_1_p_new-L_1_p_real


L_3_here_31 = L_3_old-L_3_real
L_2_here_31 = L_2_old-L_2_real
L_1_here_31 = L_1_old-L_1_real
L_3_p_here_31 = L_3_p_old-L_3_p_real
L_2_p_here_31 = L_2_p_old-L_2_p_real
L_1_p_here_31 = L_1_p_old-L_1_p_real
...
c = ChainConsumer()
parameters=[ r"$L_{3} \: (10^{-8} s)$", r"$L_{2} \: (10^{-8} s)$",r"$L_{1} \: (10^{-8} s)$", r"$L^{'}_{3} \: (10^{-8} s)$",r"$L^{'}_{2} \: (10^{-8} s)$",r"$L^{'}_{1} \: (10^{-8} s)$"]
c.add_chain(data_plot, parameters=parameters,color='green',name='Oversampled Data Generation')
c.add_chain(data_plot_31, parameters=parameters,color='blue',name='Interpolated-only data')
c.configure(sigmas=[0,1.645],spacing=1.0,kde=False,serif=True,smooth=1,usetex=True)
...
fig =c.plotter.plot(chains=['Oversampled Data Generation','Interpolated-only data'],filename='comparison_chain_consumer.png', truth=[0e1,0e1,0e1,0e1,0e1,0e1],display=False,legend=True)
print('ax')
print(fig.axes[6])
...
#corner py method
#----------------------------------------------------------------------------
#purple_line = mlines.Line2D([], [], color='purple', label='Unequal Arm XYZ')

green_line = mlines.Line2D([], [], color='green', label='Oversampled Data Generation')
blue_line = mlines.Line2D([], [], color='blue', label='Interpolated-only data')




#blue_line = mlines.Line2D([], [], color='blue', label = 'fs = 3 Hz Unequal Arm including secondary noises')
#green_line = mlines.Line2D([], [], color='green', label = 'fs = 3 Hz LFN-only')
#purple_line = mlines.Line2D([], [], color='purple', label = 'true values as initials chain #3')
...
#plt.savefig(dir+'corner_plot_compare_data_generation.png')
plt.show()
'''
...
plt.show()
'''
plt.hist(chi_2_new,label='Oversampled Data Generation',color='green',alpha=0.5)
plt.hist(chi_2_old,label='Interpolated-only data',color='blue',alpha=0.5)
...
#plt.xlabel()
plt.legend()
plt.xlabel(r'$\sum\limits^{f_{\mathrm{max}}}_{i=f_{\mathrm{min}}} \chi^{2}_{i}$')
plt.savefig(dir+'chi_2_compare_data_generation.png')
plt.show()
