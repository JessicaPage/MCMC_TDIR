...
def noise_T(f):
	...
	#return 2.0*(np.power((1.0+2.0*np.cos(2.0*np.pi*f_band*L_arm)),2))*(4.0*np.power(np.sin(np.pi*f_band*L_arm),2)*S_proof_mass+S_optical_path)
...
#re-derived acceleration noise components from Rigid Rotation TDI equation in (Tinto et al. 2004)
	...
	xx_OMS = 4*Sy_OP*(2-np.cos(2*np.pi*f_band*(L3+L3p))-np.cos(2*np.pi*f_band*(L2+L2p)))
	yy_OMS = 4*Sy_OP*(2-np.cos(2*np.pi*f_band*(L1+L1p))-np.cos(2*np.pi*f_band*(L3+L3p)))
	zz_OMS = 4*Sy_OP*(2-np.cos(2*np.pi*f_band*(L2+L2p))-np.cos(2*np.pi*f_band*(L1+L1p)))
	...
	yz = (4*Sy_PM+Sy_OP)*np.exp(-2j*np.pi*f_band*(L1p+L2+L2p))*(1+np.exp(2j*np.pi*f_band*(L1+L1p)))*(-1+np.exp(2j*np.pi*f_band*(L2+L2p)))*(-1+np.exp(2j*np.pi*f_band*(L3+L3p)))
...
def likelihood_analytical(x_combo_f,y_combo_f,z_combo_f,delay_array):
	...
	m,n,p,ax,ay,bx,by,cx,cy = covariance_unequal_arm(delay_array)
...
number_chain = 50000
#number_chain  = 5000
checkfile = open('chainfile_noise_matrix_unequal_arm_testing.dat','w')
...
old_likelihood,determ_here,chi_2_here = likelihood_analytical(x_combo_initial,y_combo_initial,z_combo_initial,np.array([initial_L_1,initial_L_1_p,initial_L_2,initial_L_2_p,initial_L_3,initial_L_3_p]))
print('likelihood')
...
while k <= number_chain:
	...
	new_likelihood,new_determ_here,new_chi_2_here = likelihood_analytical(x_combo_old,y_combo_new,z_combo_new,np.array([L_1_draw,old_L_1_p,old_L_2,old_L_2_p,old_L_3,old_L_3_p]))
	...
	new_likelihood,new_determ_here,new_chi_2_here = likelihood_analytical(x_combo_old,y_combo_new,z_combo_new,np.array([old_L_1,L_1_p_draw,old_L_2,old_L_2_p,old_L_3,old_L_3_p]))
	...
	new_likelihood,new_determ_here,new_chi_2_here = likelihood_analytical(x_combo_new,y_combo_old,z_combo_new,np.array([old_L_1,old_L_1_p,L_2_draw,old_L_2_p,old_L_3,old_L_3_p]))
	...
	new_likelihood,new_determ_here,new_chi_2_here = likelihood_analytical(x_combo_new,y_combo_old,z_combo_new,np.array([old_L_1,old_L_1_p,old_L_2,L_2_p_draw,old_L_3,old_L_3_p]))
	...
	new_likelihood,new_determ_here,new_chi_2_here = likelihood_analytical(x_combo_new,y_combo_new,z_combo_old,np.array([old_L_1,old_L_1_p,old_L_2,old_L_2_p,L_3_draw,old_L_3_p]))
	...
	new_likelihood,new_determ_here,new_chi_2_here = likelihood_analytical(x_combo_new,y_combo_new,z_combo_old,np.array([old_L_1,old_L_1_p,old_L_2,old_L_2_p,old_L_3,L_3_p_draw]))
